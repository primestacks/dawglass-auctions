...
function FeaturedAuctionCard({ auctionData }) {
  const { image, title, currentBid, bids, views, isLive } = auctionData;
  ...
  return (
    ...
    <div className="bg-[#ffffff] relative shadow-md rounded-lg w-full md:max-w-[300px] h-auto text-left md:max-h-[430px]">
      ...
      <div className="p-4">
        <h2 className="font-Inter my-2">{title}</h2>
        <div className="flex justify-between  my-2">
          <div className="flex flex-col">
            <span>Current Bid</span>
            <span className="text-[#2094F3]">${currentBid}.00</span>
          </div>
          <div className="flex flex-col">
            <span>{bids} bids</span>
        ...
        <button className="bg-[#2094F3] inline-block cursor-pointer w-full py-3 rounded mt-3 text-[#ffffff] font-Inter font-semibold">
          <Link to={`/auction/${auctionData.id}/${auctionData.slug}`}>
            Place a bid
          </Link>
        </button>
      </div>
